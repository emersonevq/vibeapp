...
interface CreateStoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (content: string, mediaData?: any, storyDuration?: number, backgroundColor?: string) => void;
}

export function CreateStoryModal({ isOpen, onClose, onSubmit }: CreateStoryModalProps) {
  const [storyType, setStoryType] = useState<'text' | 'photo' | 'video' | 'music'>('text');
  ...
  const handleSubmit = async () => {
    ...
    if ((storyType === 'photo' || storyType === 'video' || storyType === 'music') && !mediaFile) return;
      ...
      <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900">Criar Story</h2>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <Image className="w-6 h-6 mx-auto mb-2 text-green-600" />
          ...
          {/* Upload de Mídia */}
          {(storyType === 'photo' || storyType === 'video' || storyType === 'music') && (
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                ...
                <label htmlFor="media-upload" className="cursor-pointer">
                  ...
                  <p className="text-sm text-gray-600">
                    Clique para selecionar {storyType === 'photo' ? 'uma foto' : storyType === 'video' ? 'um vídeo' : 'uma música'}
                  </p>
                </label>
              </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {storyType === 'text' ? 'Conteúdo do Story' : 'Legenda (opcional)'}
            </label>
            <textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder={storyType === 'text' ? 'O que você quer compartilhar?' : 'Adicione uma legenda...'}
          ...
          <div className="flex space-x-3">
            ...
            <button
              ...
              disabled={
                ...
                ((storyType === 'photo' || storyType === 'video' || storyType === 'music') && !mediaFile)
              }
